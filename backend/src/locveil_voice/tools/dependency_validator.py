...
Usage:
    python -m locveil_voice.tools.dependency_validator --file irene/providers/audio/sounddevice.py --class SoundDeviceAudioProvider --platform linux.ubuntu
    python -m locveil_voice.tools.dependency_validator --validate-all --platform linux.alpine
    python -m locveil_voice.tools.dependency_validator --validate-all --platforms linux.ubuntu,linux.alpine,macos,windows
"""
...
class DependencyValidator:
    ...
    def collect_all_dependencies(self) -> Dict[str, Dict[str, Any]]:
        ...
        for namespace, providers in entry_points.items():
            if not namespace.startswith("locveil_voice."):
                continue
...
Examples:
  ...
  python -m locveil_voice.tools.dependency_validator \\

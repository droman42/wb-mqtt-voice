...
class ReportBundleCollector:
    ...
    def _todays_logs(self) -> List[Tuple[str, bytes]]:
        ...
        if not self.logs_dir.exists():
            return out
        # Rotated siblings live in the `locveil_voice.log.<stamp>.log` family (BUG-30): daily
        # rotation stamps `locveil_voice.log.YYYYMMDD.log`, startup rollover `locveil_voice.log.YYYYMMDD_HHMMSS.log`.
        candidates = [self.logs_dir / "locveil_voice.log"]
        candidates += sorted(self.logs_dir.glob(f"locveil_voice.log.{today}*.log"))

...
1. **Настройка окружения**: `uv add tensorflow librosa numpy pyyaml huggingface_hub`
...
- **Параметры MFCC**: окно 30 мс (480 сэмплов), шаг 10 мс (160), **n_fft=512** (zero-pad окна), n_mels=40, n_mfcc=40, DCT-II
- **Архитектура модели**: medium-12-bn (12 × Conv2D 3×1 + BatchNorm + GlobalAveragePooling)
- **Квантизация**: INT8 квантизация (автоматическая, оптимизированная для ESP32)

## Гарантии Совместимости ESP32

Совместимость **проверяется автоматически** (форма входа, INT8-квантизация, набор поддерживаемых операций, размер модели). Сборка/валидация падает при несоответствии:

| **Требование** | **Значение** | **Статус** |
|----------------|--------------|------------|
| **Входная Форма** | `[1, 49, 40]` или `[1, 49, 40, 1]` (читается из модели на ESP32) | ✅ Проверяется |
| **Размер Модели** | ≤140КБ | ✅ Валидировано |
| **Время Инференса** | ≤25мс | ✅ Оптимизировано |
| **Арена Тензоров** | Подбирается по телеметрии (`arena_used_bytes()` + 10–15%); ориентир 120–200 КБ | ✅ Измеряется |
| **Архитектура** | medium-12-bn (Conv2D + GAP) | ✅ Реализовано |
...
graph TD
    subgraph "Модель medium-12-bn"
        A["Входные MFCC<br/>(1, 49, 40, 1)"] --> B["Conv2D (3×1) Block 1<br/>filters: 8"]
        B --> C["BatchNorm + ReLU"]
        C --> D["Conv2D (3×1) Block 2<br/>filters: 16"]
        D --> E["BatchNorm + ReLU"]
        E --> F["Conv2D (3×1) Block 3<br/>filters: 16"]
        F --> G["BatchNorm + ReLU"]
        G --> H["... 9 дополнительных блоков"]
        H --> I["Conv2D (3×1) Block 12<br/>filters: 32"]
        I --> J["GlobalAveragePooling2D (MEAN)"]
        J --> K["Dense Layer<br/>units: 1"]
        K --> L["Sigmoid<br/>Выход (0,1)"]
    end
    
    subgraph "Обработка Аудио"
        M["16kHz Аудио"] --> N["MFCC Экстракция<br/>40 коэффициентов<br/>n_fft=512, DCT-II"]
        N --> O["Окно 30 мс, шаг 10 мс<br/>49 фреймов (~490 мс контекст)"]
        O --> A
    end
...
| **Ложные Срабатывания** | ≤2 в час на 3ч фоновой речи | 
| **F1 Score** | ≥0.93 |
| **Latency** | Реал-тайм на ESP32-S3 @ 240МГц |

### Порог Решения и Квант-параметры

- Порог решения (**decision threshold**) вычисляется офлайн по валидационному набору для максимизации F1 и сохраняется вместе с моделью.
- В экспортируемую конфигурацию добавляются квант-параметры входа/выхода: `input_scale`, `input_zero_point`, `output_scale`, `output_zero_point`.
- На ESP32 прошивка **не использует жёсткое 0.5** — применяется обученный порог, а квант-параметры используются для правильной INT8 (де)квантизации.
...
uv add tensorflow librosa numpy pyyaml huggingface_hub

# Опционально: Для валидации аудио и визуализации
uv add soundfile matplotlib seaborn datasets
